...
class ReminderDto { @IsString() templateId!:string; @IsDateString() sendAt!:string; @IsString() subject!:string; @IsString() body!:string; }
...
@Controller('organizations/:organizationId') export class CommunicationsController {
  ...
  @Post('events/:eventId/reminders') remind(@CurrentUser()u:AuthenticatedUser,@Param('organizationId')o:string,@Param('eventId')e:string,@Body()d:ReminderDto){return this.c.scheduleReminder(u.id,o,e,d.templateId,new Date(d.sendAt),d.subject,d.body)}
