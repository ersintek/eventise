...
export function renderEmailBody(input:string,variables:Record<string,string>){
  ...
  return escaped
    .replace(variablePattern,(_,key:string)=>renderBodyVariable(key,variables))
    .replace(/\r?\n/g,'<br>');
}
...
function renderBodyVariable(key:string,variables:Record<string,string>){const value=variables[key];if(value===undefined)throw new BadRequestException(`Eksik e-posta değişkeni: ${key}`);const escaped=escapeHtml(value);return urlVariables.has(key)&&/^https?:\/\//i.test(value)?`<a href="${escaped}">${escaped}</a>`:escaped}
...
const urlVariables=new Set(['event.public_url','event.participant_url','certificate.url']);
