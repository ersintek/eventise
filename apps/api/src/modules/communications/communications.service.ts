...
export class CommunicationsService implements OnModuleInit {
  ...
  async queueRegistrationMessage(input: { organizationId:string; templateKey:string; recipient:string; participantFirstName:string; participantFullName:string; organizationName:string; eventName:string; eventStart:Date; eventEnd?:Date; eventLocation?:string; eventPublicUrl:string; certificateUrl?:string; subjectOverride?:string; bodyOverride?:string }) {
    ...
    const message = await this.prisma.emailMessage.create({ data: { templateId:template.id, recipient:input.recipient, subject:renderTemplate(subject, variables), body:renderEmailBody(body, variables) } });
