...
import { CreateEventDto, EventStateDto, UpdateEventDto, UpdateFaqsDto } from './dto/event.dto'; import { EventsService } from './events.service';
...
export class EventsController { constructor(@Inject(EventsService) private events: EventsService) {}
  ...
  @ApiBearerAuth() @Put('organizations/:organizationId/events/:eventId/faqs') updateFaqs(@CurrentUser()u:AuthenticatedUser,@Param('organizationId')o:string,@Param('eventId')e:string,@Body()d:UpdateFaqsDto){return this.events.updateFaqs(u.id,o,e,d.faqs);}
