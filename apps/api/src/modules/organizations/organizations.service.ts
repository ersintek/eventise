...
export class OrganizationsService {
  ...
  async create(userId: string, dto: CreateOrganizationDto) {
    ...
    const organization = await this.prisma.organization.create({ data: { name: dto.name.trim(), slug: dto.slug, description: dto.description?.trim(), contactEmail: dto.contactEmail.toLowerCase(), website: dto.website, tierId: tier.id, memberships: { create: { userId, role: 'ORGANIZATION_ADMIN' } } }, include: { tier: true, memberships: true } });
