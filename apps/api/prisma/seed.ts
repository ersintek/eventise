...
async function main() {
  await prisma.tier.upsert({
    where: { key: 'tier-1' }, update: { featureFlags: { doorRegistration: true, basicSpamProtection: true, advancedReports: true, prioritySupport: true } },
  ...
  await prisma.tier.upsert({
    where: { key: 'tier-2' }, update: { featureFlags: { doorRegistration: true, basicSpamProtection: true, advancedReports: true, prioritySupport: true } },
  ...
  await prisma.tier.upsert({
    where: { key: 'tier-3' }, update: { featureFlags: { doorRegistration: true, basicSpamProtection: true, advancedReports: true, prioritySupport: true } },
