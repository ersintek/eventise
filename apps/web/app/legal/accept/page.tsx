...
function LegalAcceptanceForm() {
  ...
  const destination = ['organization', 'participant', 'dashboard'].includes(search.get('destination') ?? '') ? search.get('destination')! : 'participant';
  const destinationPath = destination === 'organization' ? '/organization/access' : destination === 'dashboard' ? '/dashboard' : '/participant';
  ...
  return <main className="center-shell"><form className="onboarding-card" onSubmit={submit}><div className="logo dark"><b>e</b>eventise</div><p className="eyebrow">SON BİR ADIM</p><h1>Kullanım koşulları</h1><p>Koşulları kabul ettikten sonra {destination === 'organization' ? 'STK erişim durumunuza' : destination === 'dashboard' ? 'kurum panelinize' : 'katılımcı alanınıza'} yönlendirileceksiniz.</p><label className="consent"><input type="checkbox" required/><span><Link href="/legal/kullanici-sozlesmesi" target="_blank">Eventise Kullanıcı Sözleşmesi</Link>&apos;ni okudum ve kabul ediyorum.</span></label><p><Link href="/legal/kvkk-aydinlatma" target="_blank">KVKK Aydınlatma Metni</Link> kişisel verilerinizin nasıl işlendiğini açıklar.</p>{error&&<p className="error">{error}</p>}<button className="primary" disabled={busy}>{busy?'Kaydediliyor…':'Kabul et ve devam et'}</button></form></main>;
}
