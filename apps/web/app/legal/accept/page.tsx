...
function LegalAcceptanceForm() {
  ...
  useEffect(() => { void fetch('/api/backend/legal/status').then(async response => {
    ...
    else if (response.ok && (await response.json()).userTermsAccepted) router.replace(search.get('destination') === 'onboarding' ? '/onboarding' : '/participant');
  }); }, [router, search]);
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    router.push(search.get('destination') === 'onboarding' ? '/onboarding' : '/participant'); router.refresh();
  }
  return <main className="center-shell"><form className="onboarding-card" onSubmit={submit}><div className="logo dark"><b>e</b>eventise</div><p className="eyebrow">SON BİR ADIM</p><h1>Kullanım koşulları</h1><p>Eventise geliştirme aşamasındadır. Devam etmeden önce güncel koşulları inceleyin.</p><label className="consent"><input type="checkbox" required/><span><Link href="/legal/kullanici-sozlesmesi" target="_blank">Eventise Kullanıcı Sözleşmesi</Link>&apos;ni okudum ve kabul ediyorum.</span></label><p><Link href="/legal/kvkk-aydinlatma" target="_blank">KVKK Aydınlatma Metni</Link> kişisel verilerinizin nasıl işlendiğini açıklar.</p>{error&&<p className="error">{error}</p>}<button className="primary" disabled={busy}>{busy?'Kaydediliyor…':'Kabul et ve devam et'}</button></form></main>;
}
