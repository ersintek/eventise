...
type History={id:string;title:string;startsAt:string;period:'CURRENT'|'UPCOMING'|'PAST';organization:{name:string}};type Certificate={id:string;verificationCode:string;downloadUrl:string;event:{title:string;organization:{name:string}}};type Application={id:string;title:string;startsAt:string;applicationStatus:'SUBMITTED'|'PENDING'|'ACCEPTED'|'WAITLISTED'|'REJECTED';organization:{name:string}};type Modules={assessments:any[];feedback:any[];games:any[];resources:any[];notifications:any[]};
...
export function ParticipantApplicationList({applications}:{applications:Application[]}){
  ...
  return <section className="participant-events participant-applications"><div className="section-heading"><div><p className="eyebrow">BAŞVURULARIN</p><h2>Etkinlik durumların</h2></div><span>{applications.length} başvuru</span></div>{applications.map(application=>{const accepted=application.applicationStatus==='ACCEPTED';const content=<><span className={'pill application-status '+application.applicationStatus.toLowerCase()}>{applicationLabel[application.applicationStatus]}</span><div><h3>{application.title}</h3><small>{application.organization.name}</small><p>{formatDateLong(application.startsAt)}</p>{accepted&&<small className="application-access">Etkinlik alanına girebilirsin.</small>}</div>{accepted&&<span className="event-arrow">→</span>}</>;return accepted?<Link href={'/participant/event/'+application.id} key={application.id} className="participant-event-link">{content}</Link>:<article key={application.id} className="participant-event-link participant-event-static" aria-label={`${application.title}: ${applicationLabel[application.applicationStatus]}`}>{content}</article>})}</section>;
}
      ...
      <div className="participant-modules">
        ...
        {pendingAssessments.length>0&&(
          <details className="module-block" open={firstOpenKey==='assessments'}>
            <summary><span className="module-icon">01</span> Tamamlanacak testler{pendingAssessments.length>0&&<span className="module-count">{pendingAssessments.length}</span>}</summary>
