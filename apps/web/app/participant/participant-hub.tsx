...
const applicationLabels: Record<string, string> = { SUBMITTED:'Başvurun alındı', PENDING:'Başvurun değerlendiriliyor', ACCEPTED:'Etkinliğe git', WAITLISTED:'Yedek listedesindesiniz', REJECTED:'Başvuru sonucu açıklandı' };
...
export function ParticipantHub({ me, history, upcomingEvents, certificates, initialFollows, initialFollowingEvents, legal, initialNotifications, eventTasks }: { me: Me; history: History[]; upcomingEvents: UpcomingEvent[]; certificates: any[]; initialFollows: Follow[]; initialFollowingEvents: FollowingEvent[]; legal: LegalStatus; initialNotifications: Notification[]; eventTasks: EventTask[] }) {
  ...
  const primaryRegistration = upcomingEvents.find(item => item.registration?.applicationStatus === 'ACCEPTED') ?? upcomingEvents.find(item => item.registration && item.registration.applicationStatus !== 'REJECTED');
  ...
  return <div className="participant-shell participant-redesign">
    ...
    {tab === 'home' && <section className="participant-home" aria-label="Katılımcı ana sayfası">
      <section className="participant-priority" aria-labelledby="today-title"><div><p className="eyebrow">BUGÜN</p><h2 id="today-title">{primaryTask ? 'Tamamlaman gereken bir iş var.' : primaryRegistration ? 'Sıradaki adımın hazır.' : 'Şu an bekleyen bir işlemin yok.'}</h2><p>{primaryTask ? `${primaryTask.title} için ${primaryTask.pending} görevin bekliyor. Önce bunları tamamlayarak etkinlik sürecini ilerletebilirsin.` : primaryRegistration ? `${primaryRegistration.title} için durumunu ve etkinlik bilgilerini buradan takip edebilirsin.` : 'Yeni etkinlikleri inceleyerek sana uygun olana başvurabilirsin.'}</p></div>{primaryTask ? <Link className="primary link-button" href={`/participant/event/${primaryTask.eventId}`}>Görevleri aç <span aria-hidden="true">→</span></Link> : primaryRegistration ? <Link className="primary link-button" href={`/participant/event/${primaryRegistration.id}`}>{applicationLabels[primaryRegistration.registration?.applicationStatus ?? 'ACCEPTED'] ?? 'Etkinliği aç'} <span aria-hidden="true">→</span></Link> : <button className="primary" onClick={go('discover')}>Etkinlikleri keşfet <span aria-hidden="true">→</span></button>}</section>
