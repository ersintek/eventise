...
type History = { id: string; title: string; startsAt: string; timezone: string; period: 'CURRENT'|'UPCOMING'|'PAST'; organization: Organization };
type Follow = { id: string; organizationId: string; organization: Organization };
type FollowingEvent = { id: string; title: string; slug: string; summary?: string; startsAt: string; timezone: string; organization: Organization };
type UpcomingEvent = { id:string; title:string; slug:string; summary?:string; startsAt:string; endsAt:string; timezone:string; format:string; venueName?:string; registrationStatus:string; followed:boolean; registration:{id:string;applicationStatus:string}|null; organization:Organization };
...
export function ParticipantHub({ me, history, upcomingEvents, certificates, initialFollows, initialFollowingEvents, legal, initialNotifications, eventTasks }: { me: Me; history: History[]; upcomingEvents: UpcomingEvent[]; certificates: any[]; initialFollows: Follow[]; initialFollowingEvents: FollowingEvent[]; legal: LegalStatus; initialNotifications: Notification[]; eventTasks: EventTask[] }) {
  ...
  const applications = useMemo(() => {
    const byEventId = new Map(upcomingEvents.filter(item => item.registration).map(item => [item.id, { id:item.id, title:item.title, startsAt:item.startsAt, timezone:item.timezone, organization:item.organization, applicationStatus:item.registration!.applicationStatus as 'SUBMITTED'|'PENDING'|'ACCEPTED'|'WAITLISTED'|'REJECTED' }]));
    for (const event of activeHistory) if (!byEventId.has(event.id)) byEventId.set(event.id, { id:event.id, title:event.title, startsAt:event.startsAt, timezone:event.timezone, organization:event.organization, applicationStatus:'ACCEPTED' });
