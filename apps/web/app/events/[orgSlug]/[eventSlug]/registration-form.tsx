...
const statusMessage:Record<string,string>={SUBMITTED:'Başvurunuz alındı.',PENDING:'Başvurunuz değerlendirmeye alındı.',ACCEPTED:'Kaydınız tamamlandı.',WAITLISTED:'Kontenjan dolu olduğu için yedek listeye alındınız.',REJECTED:'Başvurunuz kabul edilmedi.'};
export function RegistrationForm({ orgSlug, eventSlug, open, consents, fields, session }: { orgSlug: string; eventSlug: string; open: boolean; consents: Consent[]; fields: RegistrationField[]; session: { user: { email: string; firstName: string; lastName: string }; registration: { applicationStatus: string } | null } | null }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); setBusy(true); setMessage(''); const form = event.currentTarget, data = new FormData(form), consentVersionIds = visibleConsents.filter(item => data.get(`consent-${item.definition.versions[0]?.id}`)).map(item => item.definition.versions[0].id), answers = Object.fromEntries(fields.map(field => [field.key, field.type === 'checkbox' ? data.get(field.key) === 'on' : data.get(field.key)])); const payload = { firstName: data.get('firstName'), lastName: data.get('lastName'), email: data.get('email'), answers, consentVersionIds, createAccount: user ? false : data.get('createAccount') === 'on' }; try { const response = await fetch(`/api/public/events/${orgSlug}/${eventSlug}/registrations`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(payload) }), result = await response.json(); setMessage(response.ok ? statusMessage[result.status] ?? 'Başvurunuz alındı.' : (Array.isArray(result.message) ? result.message.join(' ') : result.message ?? 'Başvuru gönderilemedi.')); if (response.ok) form.reset(); } catch { setMessage('Bağlantı kurulamadı. Bilgileriniz korunuyor; lütfen yeniden deneyin.'); } finally { setBusy(false); } }
  ...
  if (existing) return <aside className="registration-card" id="registration"><p className="eyebrow">BAŞVURU DURUMU</p><h2>{statusLabel[existing.applicationStatus] ?? existing.applicationStatus}</h2><p className="registration-explainer">{statusMessage[existing.applicationStatus] ?? 'Başvurunuz kaydedildi.'}</p><p className="participant-notice"><b>{user?.email}</b></p></aside>;
  return <form className="registration-card" id="registration" onSubmit={submit}><p className="eyebrow">KAYIT FORMU</p><h2>Yerinizi ayırın</h2><p className="registration-explainer">Bilgilerinizi paylaşın; kayıt durumunuz e-postayla bildirilsin.</p>
    ...
    <button className="primary" disabled={busy||missingVersionConsents.length>0}>{busy ? 'Gönderiliyor…' : 'Başvuruyu gönder'}</button>
  </form>;
}
