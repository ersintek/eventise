...
export default async function EventManagement({params}:{params:Promise<{eventId:string}>}) {
  ...
  const [registrations,forms,templates,consents,reminders]=await Promise.all([
    ...
    optionalApi<unknown[]>(`organizations/${organization.id}/events/${eventId}/reminders`,token,[]),
  ]);
